...
public class Dinic {
  ...
  public int findMaximumFlow(int source, int sink) {
    ...
    while (bfs(source, sink, level)) {
      ...
      while ((pushed = dfs(source, sink, Integer.MAX_VALUE, level, next)) > 0) {
        maxFlow += pushed;
      }
    }
    return maxFlow;
  ...
  private boolean bfs(int source, int sink, int[] level) {
    ...
    while (!queue.isEmpty()) {
      ...
      for (Edge edge : adj.get(node)) {
        if (edge.getWeight() > 0 && level[edge.getTo()] == -1) {
  ...
  private int dfs(int source, int sink, int flow, int[] level, int[] next) {
    if (source == sink) {
      return flow;
    }

    for (; next[source] < adj.get(source).size(); next[source]++) {
      Edge edge = adj.get(source).get(next[source]);
      if (edge.getWeight() > 0 && level[edge.getTo()] == level[source] + 1) {
        int pushed = dfs(edge.getTo(), sink, Math.min(flow, edge.getWeight()), level, next);
        if (pushed > 0) {
          edge.setWeight(edge.getWeight() - pushed);
          edge.getReverseEdge().setWeight(edge.getReverseEdge().getWeight() + pushed);
          return pushed;
        }
      }
    }
