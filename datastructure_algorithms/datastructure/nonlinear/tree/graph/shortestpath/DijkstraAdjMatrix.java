...
public class DijkstraAdjMatrix {
  ...
  public DijkstraAdjMatrix() {
  }

  public DijkstraAdjMatrix(int vertices) {
  ...
  public void dijkstraShortestPath(int source) {
    ...
    for (int i = 0; i < vertices; i++) {
      ...
      for (int j = 0; j < vertices; j++) {
        if (!visited[j] && matrix[node][j] != 0 && distance[node] != Integer.MAX_VALUE
    ...
    for (int i = 0; i < distance.length; i++) {
      if (distance[i] == Integer.MAX_VALUE) {
        ...
        System.out.println(i + ": " + distance[i] + "|  path > " + printPath(parent, i));
      }
    }
  ...
  public static void main(String[] args) {
    DijkstraAdjMatrix graph = new DijkstraAdjMatrix(5);
