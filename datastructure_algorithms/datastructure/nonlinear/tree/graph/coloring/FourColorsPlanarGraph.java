...
public class FourColorsPlanarGraph {

  private List<List<Edge>> adj;
  private int vertices; // No. of vertices

  public FourColorsPlanarGraph(int vertices) {
  ...
  public int[] colorGraph4() {
    int[] colors = new int[vertices];
    Arrays.fill(colors, -1); // -1 means uncolored

    for (int i = 0; i < vertices; i++) {
      if (colors[i] == -1) {
        assignColor(i, colors);
      }
    }
    return colors;
  }

  private boolean assignColor(int node, int[] colors) {
    boolean[] used = new boolean[4]; // 4 colors
    ...
    // Recursive assignment for neighbors
    for (Edge edge : adj.get(node)) {
      if (colors[edge.getTo()] == -1) {
        if (!assignColor(edge.getTo(), colors)) {
          return false;
        }
      }
    }
  ...
  public static void main(String[] args) {
    FourColorsPlanarGraph graph = new FourColorsPlanarGraph(6);
    graph.addEdge(0, 1);
    graph.addEdge(0, 2);
    graph.addEdge(0, 3);
    graph.addEdge(1, 2);
